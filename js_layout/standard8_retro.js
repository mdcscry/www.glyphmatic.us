...
var newRule='@-' + browserPrefix +'-keyframes fadeIn{0%{opacity:0;}100%{opacity:.30;}}';
cssStyle.insertRule(newRule,cssRules.length);
...
for (i=1;i<=window.divCounter;i++){
    var newRule='#myid'+i+' {-' +  browserPrefix +'-transition-property:color,textShadow;-' +  browserPrefix +'-transition-duration:3s,5s;background:transparent;padding:0px;margin:0px; text-align:center;height:100%;width:100%;position:absolute;font-size:1400px;top:-25%;left:-2.5%;}';   
    cssStyle.insertRule(newRule,cssRules.length);
    var newRule='#myid'+i+'.display {-' +  browserPrefix +'-transition-property:color,textShadow;-' +  browserPrefix +'-transition-duration:3s,5s;-' +  browserPrefix +'-animation-name:fadeIn; -' +  browserPrefix +'-animation-duration:.25s;-' +  browserPrefix +'-animation-timing-function:ease-in-out;-' +  browserPrefix +'-animation-delay:0s;-' +  browserPrefix +'-animation-iteration-count:1; -' +  browserPrefix +'-animation-direction:normal;-' +  browserPrefix +'-animation-play-state:running;-' +  browserPrefix +'-animation-fill-mode:both;pointer-events: none;}';
    cssStyle.insertRule(newRule,cssRules.length);
